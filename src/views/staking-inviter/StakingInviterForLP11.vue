        ...
        <v-col md="6">
          ...
          <v-card class="fill-width mt-10">
            <v-card outlined>
              ...
              <v-divider></v-divider>
              <v-card-text v-if="!capReached && isOpen && isHasInviter">
                <form>
                  <v-card-text>
                    <v-text-field
                      :label="
              ...
              <v-card-text v-else>
                <v-row align="center">
                  <v-col
                    v-if="isOpen && !isHasInviter"
                    class="subtitle-1"
                    cols="12"
                  >
                    {{
                      $t("You have to accept invitation to be able to stake")
                    }}
                  </v-col>
...
import {
  ...
  DSTAddress,
  ZeroAddress,
  StakingInviterForLPTokenAddress11,
  StakingInviterForLPContractAddress11,
...
import Relationship_ABI from "@/constants/contractJson/Relationship_abi.json";
...
export default {
  ...
  methods: {
    ...
    async getRelationship() {
      ...
      const inviter = await contract.methods
        .getInviterInfoByInvitee(toChecksumAddress(this.address))
        .call({ from: StakingInviterForLPContractAddress11 });
      this.isHasInviter = ZeroAddress !== inviter.inviterToken;
    },
    // 获取账号信息
    async getAccountAssets() {
      ...
      const allowance = await contractERC20DAO.methods
        .allowance(this.address, StakingInviterForLPContractAddress11)
        .call();
    ...
    // 跳转历史记录
    gotoHistory() {
      this.$router.push({ path: "/staking/lp/11/history" });
    }
  }
};
</script>
